...
class TableFusion(nn.Module):
    ...
    def forward(self, table_hidden: Optional[torch.Tensor] = None,
        ...
        hidden_states_retrieve_proj = self.retrieve_embed_proj(table_hidden.cuda())
        ...
        for ba, offset in enumerate(cell_span):
            ...
            chunk = torch.index_add(chunk, 0, gather_index[ba].cuda(), hidden_states_retrieve_proj[ba, :].cuda())
            ...
            has_connect_lable = (span_label[ba] != 0).int()
            tmp_connect_loss = self.bce_criterion(has_connect_logits, has_connect_lable.to(torch.float).cuda())
            ...
            tmp_similarity_loss = self.xe_criterion(similarity_score.reshape(-1, similarity_score.size(-1)).cuda(),
                                                    span_label[ba].reshape(-1).cuda())
            connect_loss.append(tmp_connect_loss)
            similarity_loss.append(tmp_similarity_loss)
            table_hidden = table_hidden.clone()
            for index in range(connect_span[ba].size(0)):
                ...
                try:
                    for idx, item in enumerate(tmp_connect_index):
                        if item == -1:
                            break
                        table_hidden[ba, span[0].item():span[1].item()] = table_hidden[ba, span[0].item():span[1].item()] + seq_hidden[ba][item]
                except:
                    table_hidden[ba, span[0].item():span[1].item()] = table_hidden[ba, span[0].item():span[1].item()] + seq_hidden[ba][tmp_connect_index]
...
class BartForTableFusion(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, **kwargs):
        ...
        self.e_sc = torch.tensor(self.tokenizer.convert_tokens_to_ids(["[e_source]"]))
        self.max_gen_len = 20
