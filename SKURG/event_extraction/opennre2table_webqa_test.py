...
opennre_data = json.load(
    open('../WebQA/WebQA_entities_relation_opennre.json', 'r'))
# file that contains entities and relations.

file_path = '../WebQA/WebQA_data_first_release/WebQA_train_val_small.json'
# WebQA test set
...
for key, value in data.items():
    table_context = "ROW 1 :"
    txt_facts = value['txt_posFacts'] + value['txt_negFacts']
    img_facts = value['img_posFacts'] + value['img_negFacts'] 
    entities = {}
...
pickle.dump(tables_feats,
            open('../WebQA/WebQA_opennre_tables_ofa_feats_rel.pkl',
                 'wb'))
print('save to', '../WebQA/WebQA_opennre_tables_ofa_feats_rel.pkl')
# tokenized ids to the linearized KG

pickle.dump(table_connect,
            open('../WebQA/WebQA_opennre_tables_connect_rel.pkl',
                 'wb'))
print('save to', '../WebQA/WebQA_opennre_tables_connect_rel.pkl')
# alignment between sources and head entities

pickle.dump(tables_spans,
            open('../WebQA/WebQA_opennre_tables_spans_rel.pkl',
                 'wb'))
print('save to', '../WebQA/WebQA_opennre_tables_spans_rel.pkl')
