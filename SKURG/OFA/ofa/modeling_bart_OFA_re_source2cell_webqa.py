...
class OFABart_AnsAcc(nn.Module):
    ...
    def forward(self, input_ids, input_mask, table_text, table_mask, image_feat, image_mask, image_ques_id,
        ...
        del decoder_outputs
        cross_attention_weights = torch.stack(cross_attention_weights).sum(dim=0).cuda()
        ...
        for i in range(ans_ids.size(0)):
            ...
            label = torch.cat(
                (torch.ones(cls_len - 1, dtype=torch.float32), torch.zeros(1, dtype=torch.float32))).to(
                dtype=tmp_re_logits.dtype)
            # re_logits = re_logits.float()
            tmp_loss = self.cls_criterion(tmp_re_logits.view(-1).cuda(), label.cuda())
