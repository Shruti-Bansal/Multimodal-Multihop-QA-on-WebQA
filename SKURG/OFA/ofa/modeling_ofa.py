...
class OFAAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len).cuda() + attention_mask.cuda()
...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def get_patch_images_info(self, patch_images, sample_patch_num, device):
        ...
        image_embed = self.embed_images(patch_images.cuda())
        ...
        image_pos_embed = self.embed_image_positions(image_position_ids.cuda())
    ...
    def forward_embedding(
        ...
        if token_embedding is None:
            token_embedding = self.embed_tokens(input_ids.cuda())
        ...
        if image_embed is not None:
            ...
            if self.type_embedding is not None:
                image_x += self.type_embedding(input_ids.new_ones(image_x.size()[:2]).cuda())
    ...
    def forward(
        ...
        if token_type_ids is not None:
            type_embeddings = self.type_embedding(token_type_ids.cuda())
        else:
            type_embeddings = None
        pos_embed = self.embed_positions(new_arange(input_ids).cuda())
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if patch_images_2 is not None:
                ...
                self_attn_bias[:, :, :x.size(1) - input_ids.size(1), :x.size(1) - input_ids.size(1)] += \
                    self.get_image_rel_pos_bias(image_position_ids.cuda(), idx)
