...
from OFA.datasets.generative_table_connect_dataset_webqa import generative_webqa_dataset, generative_webqa_dataset_val, generative_webqa_dataset_test
...
def compute_bartscore_ParaBank(c, a, switch=False):
    ...
    bart_scorer_ParaBank.load(
        path='./OFA/utils/BARTScore/bart_score.pth')  # Please change the path to bart.pth
    c_removepunc = [normalize_text_for_bart(x) for x in c]
    a_removepunc = [normalize_text_for_bart(x) for x in a]
    if switch:
        score = np.exp(bart_scorer_ParaBank.score(c_removepunc, a_removepunc, batch_size=1))
    else:
        score = np.exp(bart_scorer_ParaBank.score(a_removepunc, c_removepunc, batch_size=1))
    return score
...
def train(args, model, tokenizer):
    ...
    val_dataset = generative_webqa_dataset_val(tokenizer, args.example_file_eval,
                                           args.text_dir,
                                           args.img_dir, args.table_path, args.connect_path, args.span_path,
                                           args.prefix_path,
                                           args.doc_stride, args.max_img_length,
                                           args.max_query_length,
                                           args.max_seq_length, is_val=True)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                ...
                scaler.scale(loss*batch['multiplier']).backward()
                # loss.backward()
            ...
            del loss, gen_loss, pointer_loss, connect_loss, similarity_loss
...
def main():
    ...
    parser.add_argument("--example_file_train",
                        default="../WebQA/WebQA_data_first_release/WebQA_train_val_small.json",
    ...
    parser.add_argument(
        ...
        default='squad_pretrain_ofa/checkpoint-5-14000-em-65.2665-f1-76.5692-pre-100.0000-re-100.0000',
        type=str,
        help="Path to pretrained OFA on SQUAD 2.0 ",
    )
    parser.add_argument(
        "--bart_squad_model_name_or_path",
        default='squad_pretrain_bart/checkpoint-5-15000-em-78.8630-f1-88.2333-pre-100.0000-re-100.0000',
        type=str,
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        val_dataset = generative_webqa_dataset_test(tokenizer, args.example_file_test,
                                                    args.test_text_dir,
                                                    args.test_img_dir, args.table_path_test, args.connect_path_test,
                                                    args.span_path_test,
