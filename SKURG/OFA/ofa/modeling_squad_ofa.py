...
class OFADecoder_pointer(OFAPreTrainedModel):
    ...
    def forward(
        ...
        tgt_pos_embed = self.embed_positions(token_position_idx.cuda())
        ...
        x = self.embed_scale * self.embed_tokens(input_ids.cuda())
...
class OFAModel_squad(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, **kwargs):
        ...
        self.b_sc = torch.tensor(self.tokenizer.encode("[b_source]", add_special_tokens=False))
        self.e_sc = torch.tensor(self.tokenizer.encode("[e_source]", add_special_tokens=False))
        self.max_gen_len = 20
    ...
    def forward(self, input_ids, input_mask, ans_ids, attn_mask, ans_loss_mask, input_token_type,
        ...
        ans_shift_labels = ans_labels[..., 1:].contiguous().cuda()
        shift_logits = lm_logits[..., :-1, :].contiguous().cuda()
    ...
    def evaluate(self, input_ids, input_mask, ans_ids, attn_mask, ans_loss_mask, input_token_type
        ...
        for i in range(batch_size):
            ...
            cur_unfinished = outputs.new(1).fill_(1).cuda()
            cur_len = 0
            tokens_to_add = self.e_sc
            for index in range(self.max_gen_len - 1):
                dec_input_id = torch.cat((dec_input_id.cuda(), tokens_to_add.unsqueeze(0).cuda()), -1)
                gpt_outputs, lm_logits = self.decoder(input_ids=dec_input_id,
                                                      encoder_hidden_states=encoder_hidden,
                ...
                gen_label = torch.argmax(lm_logits, dim=-1).squeeze(-1).cuda()
