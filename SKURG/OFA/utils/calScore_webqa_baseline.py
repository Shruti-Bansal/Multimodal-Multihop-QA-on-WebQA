...
def compute_bartscore_ParaBank(c, a, switch=False):
    bart_scorer_ParaBank = BARTScorer(device='cpu', checkpoint='./bart-large-cnn')
    bart_scorer_ParaBank.load(
        path='./bart_score.pth')  # Please change the path to bart.pth
...
golden_res_list = []
gen_res_list = []
golden = json.load(open('../WebQA/WebQA_data_first_release/WebQA_train_val.json', 'r'))
...
for key, value in pred.items():
    ...
    # if len(golden[key]['txt_posFacts']) == 0:
    #     continue
    if golden_ans[0] == '"':
